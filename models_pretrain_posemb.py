...
class PACA(nn.Module):
    ...
    def generate_main_orientation(self, k_x_attn, polar_pos):
        ...
        new_polar = polar_pos - repeat(main_ori, 'b h k_l-> b h k_l x_l', x_l=x_l)
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., polar_bins=8, act_layer=nn.GELU, norm_layer=nn.LayerNorm):
        super().__init__()
...
class PamaViT(nn.Module):
    """ Positioan-aware Masked Autoencoder with VisionTransformer backbone
    """

    def __init__(self, num_kernel=64, kernel_drop=0.2, in_chans=256,
                 embed_dim=1024, depth=24, num_heads=16,
                 decoder_embed_dim=512, decoder_depth=8, decoder_num_heads=16,
                 mlp_ratio=4., polar_bins=8, norm_layer=nn.LayerNorm, norm_pix_loss=False):
        super().__init__()
        ...
        self.decoder_blocks = nn.ModuleList([
            Block(decoder_embed_dim, decoder_num_heads, mlp_ratio, qkv_bias=True, qk_scale=None, norm_layer=norm_layer, polar_bins=polar_bins)
    ...
    def forward_encoder(self, x, wsi_rd, wsi_polar, token_mask, kernel_mask, kernel_tokens, device, mask_ratio):
        ...
        wsi_rd_mask = torch.gather(wsi_rd.transpose(-1, -2), dim=1,
                                   index=ids_keep.unsqueeze(-1).repeat(1, 1, wsi_rd.shape[-2])).transpose(-1, -2)  # shape[b, k_l, x_l]
        ...
        wsi_polar_mask = torch.gather(wsi_polar.transpose(-1, -2), dim=1,
                                      index=ids_keep.unsqueeze(-1).repeat(1, 1, wsi_polar.shape[-2])).transpose(-1, -2)
        ...
        # 2.use zero
        # wsi_polar_mask = torch.cat(
        #     (torch.zeros(wsi_polar_mask.shape[0], wsi_polar_mask.shape[1], 1).cuda(device), wsi_polar_mask), dim=-1)

        att_mask = einsum('b i d, b j d -> b i j', token_mask.float(), kernel_mask.float())
        att_mask = torch.gather(att_mask, dim=1, index=ids_keep.unsqueeze(-1).repeat(1, 1, att_mask.shape[-1]))
        ...
        for blk in self.blocks:
            x, kernel_tokens = blk(x, kernel_tokens, wsi_rd_mask, wsi_polar_mask,
                                               att_mask=att_mask)
        x, kernel_tokens = self.norm(x), self.norm(kernel_tokens)


        return x, mask, ids_restore, kernel_tokens

    def forward_decoder(self, x, ids_restore, wsi_rd, wsi_polar, token_mask, kernel_mask, kernel_tokens, device):
        # embed tokens
        x = self.decoder_embed(x)
        kernel_tokens = self.decoder_embed(kernel_tokens)
        ...
        # append k_clstoken rd
        rd_mean = torch.mean(wsi_rd.float(), dim=-1).to(torch.int64)
        wsi_rd_mask = torch.cat((rd_mean.unsqueeze(-1), wsi_rd), dim=-1)

        # append k_clstoken polar
        # 1.use mean
        polar_mean = torch.mean(wsi_polar.float(), dim=-1).to(torch.int64)
        wsi_polar = torch.cat((polar_mean.unsqueeze(-1), wsi_polar), dim=-1)
        # 2.use zero
        # wsi_polar = torch.cat(
        #     (torch.zeros(wsi_polar.shape[0], wsi_polar.shape[1], 1).cuda(device), wsi_polar), dim=-1)
        ...
        # append cls_token mask
        att_mask = torch.cat((torch.ones(att_mask.shape[0], 1, att_mask.shape[2]).cuda(device), att_mask), dim=1)
        att_mask = repeat(att_mask.unsqueeze(1), 'b () i j -> b h i j', h=self.decoder_num_heads) < 0.5

        # apply Transformer blocks
        for blk in self.decoder_blocks:
            x, kernel_tokens = blk(x, kernel_tokens, wsi_rd_mask, wsi_polar,
                                               att_mask=att_mask)
    ...
    def forward(self, imgs, wsi_rd, wsi_polar, token_mask, kernel_mask, device, mask_ratio=0.75):
        ...
        latent, mask, ids_restore, kernel_tokens = self.forward_encoder(imgs, wsi_rd, wsi_polar, token_mask, kernel_mask,
                                                                     kernel_tokens, device, mask_ratio)


        pred = self.forward_decoder(latent, ids_restore,wsi_rd, wsi_polar, token_mask, kernel_mask, kernel_tokens, device)
